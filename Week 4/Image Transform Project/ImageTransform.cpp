...
Email: muhammad.abbass@outlook.com
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
	...
	for (unsigned x = 0; x < image.width(); x++) {
    	for (unsigned y = 0; y < image.height(); y++) {
      		HSLAPixel & pixel = image.getPixel(x, y);
      		double dist = sqrt(pow(abs(int(centerX-x)),2) + pow(abs(int(centerY-y)),2));
			if (dist<160){
				double ill = 0.5 * dist;  
				pixel.l=pixel.l*(1-(ill/100.0));
			}
			else 
			{
				pixel.l=pixel.l*(1-80.0/100.0);
			}
